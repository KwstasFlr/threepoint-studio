...
const serviceOrder=["websites","eshops","apps","tools","modeling","music","games"];
const orderedCategories=serviceOrder.map(key=>[key,categories[key]]);
Object.keys(categories).forEach(key=>delete categories[key]);
orderedCategories.forEach(([key,value])=>categories[key]=value);

const scenes=document.querySelector(".service-scenes");
if(scenes)serviceOrder.forEach(key=>{
const scene=document.getElementById("title-"+key)?.closest(".service-scene");
if(scene&&scene.parentElement===scenes)scenes.append(scene);
});

const contactChoices=document.querySelector(".contact-choices");
if(contactChoices)serviceOrder.forEach(key=>{
const card=contactChoices.querySelector('[data-service="'+key+'"]');
if(card)contactChoices.append(card);
});

const requestedCategory=new URLSearchParams(location.search).get("category");
const categoryKey=Object.hasOwn(categories,requestedCategory||"")?requestedCategory:null;
...
body>nav .contact-nav{gap:14px;justify-content:flex-end}
body>nav .contact-nav>span:first-child{width:auto;min-height:0;text-align:right}
body>nav .contact-nav .menu-lines{flex:0 0 25px}
@media(max-width:700px){body>nav .contact-nav{gap:9px}}
body:has(.hero)>nav{
position:relative;
inset:auto;
min-height:78px;
...
.logo-word{gap:0;letter-spacing:2px}
.logo-word small{
align-self:flex-end;
color:#3b8edb;
font-size:8px;
...
z-index:1;
pointer-events:none;
background:linear-gradient(0deg,var(--bg),transparent 16%,transparent 85%,var(--bg));
}
.hero-mark{display:none!important}
.hero-bottom p{max-width:560px;font-size:16px}
.hero .round-link{margin-top:2px}
...
.category-link:hover .category-arrow{transform:translate(4px,-4px)}
.category-link:hover strong{color:var(--blue)}
.portfolio-page{min-height:60vh;padding:45px var(--pad) 80px}
.portfolio-back{
display:inline-flex;
align-items:center;
min-height:44px;
...
.portfolio-back:hover{color:var(--blue)}
.portfolio-intro{max-width:760px;margin-bottom:35px}
.portfolio-intro h1{font-size:clamp(37px,4.8vw,64px);margin:18px 0;line-height:1.1}
.portfolio-intro>p:last-child{color:var(--muted);max-width:600px;font-size:16px}
.portfolio-tabs{display:flex;flex-wrap:wrap;gap:8px;margin:30px 0 40px}
.portfolio-tabs a{padding:9px 14px;background:var(--panel);color:var(--muted);font-size:13px;border-radius:3px}
.portfolio-tabs a[aria-current="page"]{background:var(--blue);color:var(--bg)}
...
.portfolio-contact{display:inline-flex;align-items:center;gap:20px;min-height:44px;color:var(--blue);font-size:14px;margin-top:20px}
@media(min-width:1500px){.hero{min-height:680px}}
@media(max-width:950px) and (min-width:701px){
.hero{min-height:560px;padding-top:70px;padding-bottom:70px}
...
.hero{display:block;min-height:0;padding:280px var(--pad) 55px}
.hero-content{width:100%;max-width:600px}
.hero h1{font-size:clamp(40px,8vw,56px)}
.hero-image{left:20%;top:0;bottom:auto;width:100%;height:260px;transform:none!important}
.hero-image img{object-fit:contain;object-position:center;mask-image:none;-webkit-mask-image:none}
...
list.setAttribute("aria-label",language==="el"?"Κατηγορίες έργων":"Project categories");
...
if(heading)translated(heading,"Explore what we create.","Δες όσα δημιουργούμε.");
}
...
const back=translated(makeElement("a","portfolio-back"),"← Back to the studio","← Επιστροφή στο studio");
back.href=homeURL.href;

const intro=makeElement("div","portfolio-intro");
intro.setAttribute("data-reveal","");
const label=translated(makeElement("p","eyebrow"),"OUR WORK","ΤΑ ΕΡΓΑ ΜΑΣ");
const title=translated(makeElement("h1"),category.en[0],category.el[0]);
const description=translated(makeElement("p"),category.en[1],category.el[1]);
...
const link=translated(makeElement("a"),item.en[0],item.el[0]);
link.href=categoryURL(key);
...
if(label)translated(label,"Explore our services","Δες τις υπηρεσίες μας");
}
...
if(url.origin===location.origin&&url.pathname.endsWith('/contact.html')){
url.searchParams.set('service',categoryKey);
link.href=url.href;
}
});
}

if(!categoryKey)document.querySelector("#projects")?.remove();

function setLanguage(value){
language=value==="el"?"el":"en";
document.documentElement.lang=language;
all("[data-en][data-el]").forEach(element=>element.textContent=element.dataset[language]);
...
],{delay:80+index*65,fill:"backwards"});
});
});
...
all(".menu-link").forEach(link=>link.addEventListener("click",closeMenu));
...
if(event.key==="Tab"){
const controls=[...overlay.querySelectorAll("a[href],button:not([disabled])")].filter(element=>element.getClientRects().length);
...
let project=document.querySelector(".contact-choice.active")?.dataset.project||"Website";
...
{opacity:1,transform:"translateY(0)"}
],{duration:1000,delay:index*130,fill:"backwards"});
});
...
function stopMotion(){
observer?.disconnect();
pending.forEach(element=>element.style.removeProperty("opacity"));
pending.clear();
...
try{setupMotion();}catch{stopMotion();}